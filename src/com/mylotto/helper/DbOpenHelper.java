...
public class DbOpenHelper extends SQLiteOpenHelper {
	...
	private DbOpenHelper(Context context, String name, CursorFactory factory,
		...
		Log.i(Constants.LOG_TAG, "Create or Open database : " + name);
	}
	...
	private static void initialize(Context context, String databaseName,
		...
		if (instance == null) {
			...
			Log.i(Constants.LOG_TAG, "Try to create instance of database ("
					+ databaseName + ")");

			instance = new DbOpenHelper(context, databaseName, null,
					DATABASE_VERSION);
			if (isReadOnly) {
			...
			Log.i(Constants.LOG_TAG, "instance of database (" + databaseName
					+ ") created !");
		}
	}
	 ...
	 */
	public static final DbOpenHelper getInstance(Context context,
			String databaseName, boolean isReadOnly) {
	...
	public void onCreate(SQLiteDatabase db) {
		Log.d(Constants.LOG_TAG, "onCreate : nothing to do");
	...
	private static void copyDataBase(Context aContext, String databaseName)
		...
		Log.i(Constants.LOG_TAG, "Check if create dir : "
				+ getDatabaseFolder(aContext));
		...
		Log.i(Constants.LOG_TAG, "Trying to copy local DB to : " + outFileName);
		...
		Log.i(Constants.LOG_TAG, "DB (" + databaseName + ") copied!");
	}
	...
	public static boolean checkDatabase(Context aContext, String databaseName) {
		...
		try {
			...
			Log.i(Constants.LOG_TAG, "Trying to connect to : " + myPath);

			checkDB = SQLiteDatabase.openDatabase(myPath, null,
					SQLiteDatabase.OPEN_READONLY);

			Log.i(Constants.LOG_TAG, "Database " + databaseName + " found!");

			checkDB.close();
		} catch (SQLiteException e) {
			Log.i(Constants.LOG_TAG, "Database " + databaseName
					+ " does not exists!");
	 ...
	 * @return : complete path
	 */
	private static String getAbsolutePathToDatabase(Context aContext,
			String databaseName) {
	...
	private static String getDatabaseFolder(Context aContext) {
		...
		if (Environment.MEDIA_MOUNTED.equals(state)
			...
			return extStorage.getAbsolutePath() + File.separator
