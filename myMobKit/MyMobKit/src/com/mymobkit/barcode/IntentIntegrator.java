	...
	public static IntentResult parseActivityResult(int requestCode, int resultCode, Intent intent) {
		if (requestCode == REQUEST_CODE) {
			if (resultCode == Activity.RESULT_OK) {
				...
				if (TextUtils.isEmpty(contents)) contents = StringUtils.EMPTY;
				
				String formatName = intent.getStringExtra("SCAN_RESULT_FORMAT");
				if (TextUtils.isEmpty(formatName)) formatName = StringUtils.EMPTY;
				
				byte[] rawBytes = intent.getByteArrayExtra("SCAN_RESULT_BYTES");

				int intentOrientation = intent.getIntExtra("SCAN_RESULT_ORIENTATION", Integer.MIN_VALUE);
				Integer orientation = intentOrientation == Integer.MIN_VALUE ? 0 : intentOrientation;
				
				String errorCorrectionLevel = intent.getStringExtra("SCAN_RESULT_ERROR_CORRECTION_LEVEL");
				if (TextUtils.isEmpty(errorCorrectionLevel)) errorCorrectionLevel = StringUtils.EMPTY;
