...
public abstract class NanoHttpd {
	...
	private static final String TAG = AppConfig.LOG_TAG_APP + ":NanoHttpd";
							...
							asyncRunner.exec(new Runnable() {
								...
								public void run() {
									...
									try {
										...
										HTTPSession session = new HTTPSession(tempFileManager, finalAccept, inputStream, outputStream);
	...
	public static class Response {
		...
		private void send(OutputStream outputStream) {
			...
			try {
				...
				pw.flush();
				if (!isStreaming) {
					int pending = data != null ? data.available() : -1; // This
																		// is
																		// to
																		// support
																		// partial
				...
				Log.w(TAG, "[send] IOException", ioe);
			...
			try {
				...
				rlen = 0;
				{
					...
					if (!socket.isClosed() && !socket.isInputShutdown())
						...
						Log.w(TAG, "[execute] - Unable to read from input stream.");
	...
	private static final class ResponseException extends Exception {
		...
		private static final long serialVersionUID = -5093975525453889233L;
